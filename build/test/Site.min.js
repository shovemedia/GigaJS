...
define("TestTransitions",["require","jquery","lib/tween/easing/EasePack","lib/tween/TweenLite","lib/tween/plugins/CSSPlugin"],function(h){h("jquery");h("lib/tween/easing/EasePack");var q=h("lib/tween/TweenLite");h("lib/tween/plugins/CSSPlugin");var h=function(b){console.log("new TestTransitions");this.giga=b},j=h.prototype;j.setTransitionManager=function(b){this.transitionManager=b};j.fadeIn=function(b){console.log("GET fadeIn fn");b.css({opacity:0});return q.to(b,this.transitionManager.duration,
...
define("test/Site","require,jquery,lib/jquery.withSelf,lib/giga/Giga,lib/giga/PreloadController,ContentRenderer,TestTransitions".split(","),function(h){var q=h("jquery");h("lib/jquery.withSelf");var j=h("lib/giga/Giga"),b=h("lib/giga/PreloadController"),p=h("ContentRenderer"),m=h("TestTransitions"),i=function(b){this.init(b)},h=i.prototype;i.hiddenTemplate='<div style="display:none;"></div>';h.init=function(g){var h=q(i.hiddenTemplate);g.append(h);this.giga=new j(g,h);this.giga.setContentRenderer(p);
